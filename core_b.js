...
function fcas_reder(l,g){const e="pt"===g,t=e?"Erro":"Error",n=e?"Não foi possível carregar a página.":"Could not load page.",a=e?"Sem conteúdo.":"No content.",r=e?"Estatísticas":"Statistics",o=l.split("/").pop().replace(/\.[^/.]+$/,"")||(e?"Conteúdo":"Content"),s=document.createElement("style");s.textContent=":root{--bg:#f8fafc;--card:#ffffff;--text:#334155;--title:#0f172a;--border:#e2e8f0;--primary:#2563eb;--danger:#dc2626;--font:system-ui,-apple-system,sans-serif;--txt-size:1.05rem}*{box-sizing:border-box;margin:0;padding:0}html,body{width:100%;max-width:100%;overflow-x:auto}body{background-color:var(--bg);color:var(--text);font-family:var(--font);line-height:1.6;padding:0;display:flex;justify-content:center;align-items:center;min-height:100vh;min-height:100dvh;user-select:none;-webkit-user-select:none;-moz-user-select:none}main{background:var(--card);border:1px solid var(--border);border-radius:16px;box-shadow:0 10px 15px -3px rgba(0,0,0,0.05);max-width: 95%;width:calc(100% - 2rem);margin:1rem;height:85vh;height:85dvh;display:grid;grid-template-rows:auto 1fr auto;position:relative;overflow:hidden !important}.p-bar{position:absolute;top:0;left:0;height:4px;background:var(--primary);width:0%;transition:width .1s ease;z-index:50}.stats-banner{background:#f1f5f9;border-bottom:2px solid var(--primary);padding:1rem 1.5rem;display:grid;grid-template-columns:repeat(auto-fit,minmax(min(130px,100%),1fr));gap:.75rem;align-items:center;opacity:0;visibility:hidden;max-height:0;transition:all .3s ease;overflow-y:auto;position:relative;z-index:30}.stats-banner.show{opacity:1;visibility:visible;max-height:250px}.b-title{grid-column:1/-1;font-size:.75rem;color:#64748b;font-weight:700;text-transform:uppercase;letter-spacing:.05em;display:flex;align-items:center;gap:.4rem;border-bottom:1px solid var(--border);padding-bottom:.4rem;margin-bottom:.25rem}article{overflow-y:auto;overflow-x:hidden;padding:2rem clamp(1rem,5vw,3rem);scroll-behavior:smooth;max-width:100%}.t-wrapper{display:flex;align-items:center;gap:.75rem;margin-bottom:1.5rem;padding-bottom:1rem;border-bottom:1px solid var(--border);max-width:100%;overflow:hidden}h1{color:var(--title);font-size:clamp(1.4rem,4vw,2rem);font-weight:800;letter-spacing:-.03em;text-transform:capitalize;overflow-wrap:anywhere;word-break:break-word}p{font-size:var(--txt-size);margin-bottom:1.25rem;text-align:justify;overflow-wrap:anywhere;word-break:break-word}a{color:var(--primary);text-decoration:none;font-weight:600;overflow-wrap:anywhere;word-break:break-all;display:inline-block;position:relative;transition:all .2s ease}a::after{content:'';position:absolute;width:100%;transform:scaleX(0);height:2px;bottom:-2px;left:0;background-color:var(--primary);transform-origin:bottom right;transition:transform .25s ease-out}a:hover{color:#1d4ed8;transform:translateY(-1px)}a:hover::after{transform:scaleX(1);transform-origin:bottom left}article::-webkit-scrollbar, .stats-banner::-webkit-scrollbar{width:6px}article::-webkit-scrollbar-track, .stats-banner::-webkit-scrollbar-track{background:#f1f5f9;border-radius:10px}article::-webkit-scrollbar-thumb, .stats-banner::-webkit-scrollbar-thumb{background:#cbd5e1;border-radius:10px}article::-webkit-scrollbar-thumb:hover, .stats-banner::-webkit-scrollbar-thumb:hover{background:#94a3b8}article, .stats-banner{scrollbar-width:thin;scrollbar-color:#cbd5e1 #f1f5f9}.dock-container{display:flex;justify-content:center;padding-bottom:1.25rem;background:linear-gradient(to top, var(--card) 70%, transparent);position:relative;z-index:40;max-width:100%}.dock{display:flex;align-items:center;gap:.5rem;background:rgba(255,255,255,0.85);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);border:1px solid var(--border);padding:.5rem .75rem;border-radius:9999px;box-shadow:0 10px 25px -5px rgba(0,0,0,0.1),0 8px 10px -6px rgba(0,0,0,0.05)}.btn-toggle{background:transparent;border:none;color:#475569;width:36px;height:36px;border-radius:9999px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .2s;font-weight:700;font-size:1.1rem;user-select:none}.btn-toggle:hover{background:#e2e8f0;color:#1e293b}#btnUp{opacity:0;visibility:hidden;transform:scale(.8);transition:opacity .2s,visibility .2s,transform .2s}#btnUp.show{opacity:1;visibility:visible;transform:scale(1)}.stats-item{display:flex;align-items:center;gap:.5rem;font-size:clamp(.8rem,2.5vw,.9rem);font-weight:600;color:#475569;overflow-wrap:anywhere}svg{width:18px;height:18px;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;fill:none;flex-shrink:0} main{overflow:auto!important;}",document.head.appendChild(s),fetch(l).then(e=>{if(!e.ok)throw new Error;return e.text()}).then(s=>{const c=s.split("\n"),d=c.filter(e=>""!==e.trim()),i=d.map(e=>{const t=e.replace(/\b((?:https?:\/\/|www\.)[^\s]+)/g,(e,t)=>{let n=t;n=n.replace(/[\/.,;!?\)]+$/,"");const a=n.startsWith("http")?n:"https://"+n;return`<a href="${a}" target="_blank" rel="noopener">${n}</a>`});return`<p>${t}</p>`}).join(""),m=new Blob([s]).size,l=m>=1048576?`${(m/(1024*1024)).toFixed(2)} MB`:`${(m/1024).toFixed(1)} KB`,v=d.length,u=s.trim().split(/\s+/).filter(e=>e.length>0).length,f=s.length,h=s.replace(/\s/g,"").length,p=e?`<div class="b-title"><svg viewBox="0 0 24 24" style="color:var(--primary)"><line x1="18" y1="20" x2="18" y2="10"></line><line x1="12" y1="20" x2="12" y2="4"></line><line x1="6" y1="20" x2="6" y2="14"></line></svg><span>${r}</span></div><div class="stats-item"><svg viewBox="0 0 24 24"><path d="M3 12h18M3 6h18M3 18h18"/></svg>Linhas: ${v}</div><div class="stats-item"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 8v8M8 12h8"/></svg>Palavras: ${u}</div><div class="stats-item"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="9" x2="15" y2="15"/><line x1="15" y1="9" x2="9" y2="15"/></svg>Com espaços: ${f}</div><div class="stats-item"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>Sem espaços: ${h}</div><div class="stats-item"><svg viewBox="0 0 24 24"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>Tamanho: ${l}</div>`:`<div class="b-title"><svg viewBox="0 0 24 24" style="color:var(--primary)"><line x1="18" y1="20" x2="18" y2="10"></line><line x1="12" y1="20" x2="12" y2="4"></line><line x1="6" y1="20" x2="6" y2="14"></line></svg><span>${r}</span></div><div class="stats-item"><svg viewBox="0 0 24 24"><path d="M3 12h18M3 6h18M3 18h18"/></svg>Lines: ${v}</div><div class="stats-item"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 8v8M8 12h8"/></svg>Words: ${u}</div><div class="stats-item"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="9" x2="15" y2="15"/><line x1="15" y1="9" x2="9" y2="15"/></svg>With spaces: ${f}</div><div class="stats-item"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>Without spaces: ${h}</div><div class="stats-item"><svg viewBox="0 0 24 24"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>Size: ${l}</div>`;document.body.innerHTML=`<main><div class="p-bar" id="pBar"></div><div class="stats-banner" id="stBanner">${p}</div><article id="artBox"><div class="t-wrapper"><svg viewBox="0 0 24 24" style="color:var(--primary);width:28px;height:28px"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg><h1>${o}</h1></div>${i||`<p>${a}</p>`}</article><div class="dock-container"><div class="dock"><button class="btn-toggle" id="fDec" style="font-size:1.3rem">-</button><button class="btn-toggle" id="fInc" style="font-size:1.1rem">+</button><button class="btn-toggle" id="btnUp"><svg viewBox="0 0 24 24"><polyline points="18 15 12 9 6 15"></polyline></svg></button><button class="btn-toggle" id="tglStats"><svg viewBox="0 0 24 24"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z"></path><circle cx="12" cy="12" r="3"></circle></svg></button></div></div></main>`;const g=document.getElementById("stBanner"),b=document.getElementById("artBox"),w=document.getElementById("btnUp"),k=document.getElementById("pBar");let x=1.05;document.getElementById("tglStats").addEventListener("click",e=>{e.stopPropagation(),g.classList.toggle("show")}),document.getElementById("fInc").addEventListener("click",()=>{x<1.6&&(x+=.1,document.documentElement.style.setProperty("--txt-size",x+"rem"))}),document.getElementById("fDec").addEventListener("click",()=>{x>.85&&(x-=.1,document.documentElement.style.setProperty("--txt-size",x+"rem"))}),w.addEventListener("click",()=>{b.scrollTop=0}),b.addEventListener("scroll",()=>{const e=b.scrollTop,t=b.scrollHeight-b.clientHeight;k.style.width=(t>0?e/t*100:0)+"%",e>20?(w.classList.add("show"),g.classList.remove("show")):(w.classList.remove("show"))}),document.addEventListener("contextmenu",e=>e.preventDefault()),document.addEventListener("keydown",e=>{(e.ctrlKey||e.metaKey)&&["c","x","u","s","a"].includes(e.key.toLowerCase())&&e.preventDefault()})}).catch(()=>{document.body.innerHTML=`<main style="border-color:#fca5a5"><header style="padding:1.5rem"><div class="h-left"><svg viewBox="0 0 24 24" style="stroke:var(--danger);width:28px;height:28px"><polygon points="7.86 2 16.14 2 22 7.86 22 16.14 16.14 22 7.86 22 2 16.14 2 7.86 7.86 2"></polygon><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg><h1 style="color:var(--danger);margin:0;border:none;font-size:1.5rem">${t}</h1></div></header><p style="padding:0 1.5rem 1.5rem">${n}</p></main>`})}


function fs_lismode_gettheme(n){const t={modern:{b:'#f8f9fa',bB:'#e2e8f0',c:'#64748b',hB:'#edf2f7',hC:'#334155',aB:'#3b82f6',aC:'#3b82f6',aBg:'#fff'},dark:{b:'#1e293b',bB:'#334155',c:'#94a3b8',hB:'#334155',hC:'#f8fafc',aB:'#6366f1',aC:'#6366f1',aBg:'#0f172a'},glass:{b:'rgba(255,255,255,0.2)',bB:'rgba(255,255,255,0.3)',c:'#475569',hB:'rgba(255,255,255,0.4)',hC:'#1e293b',aB:'#000',aC:'#000',aBg:'rgba(255,255,255,0.6)'},minimal:{b:'transparent',bB:'transparent',c:'#94a3b8',hB:'transparent',hC:'#1e293b',aB:'transparent',aC:'#1e293b',aBg:'transparent',bW:'0'},cyber:{b:'#0d0e15',bB:'#00ffcc',c:'#00ffcc',hB:'#161925',hC:'#ff007f',aB:'#ff007f',aC:'#ff007f',aBg:'#0d0e15'},sunset:{b:'#fff5f5',bB:'#fee2e2',c:'#f97316',hB:'#ffedd5',hC:'#ea580c',aB:'#ef4444',aC:'#ef4444',aBg:'#fff'},forest:{b:'#f0fdf4',bB:'#dcfce7',c:'#16a34a',hB:'#bbf7d0',hC:'#15803d',aB:'#15803d',aC:'#15803d',aBg:'#fff'},royal:{b:'#faf5ff',bB:'#e9d5ff',c:'#7c3aed',hB:'#f3e8ff',hC:'#6d28d9',aB:'#7c3aed',aC:'#fff',aBg:'#7c3aed'}};return t[n]||t.modern;} function fs_lismode_injectstyles(c,t){const s=document.createElement('style');const a=c.align==='right'?'flex-end':c.align==='center'?'center':'flex-start';const gGrid=c.gap||'30px';const gList=c.gap||'15px';s.textContent=`.lm-c{display:flex;gap:8px;margin-bottom:24px;width:100%;box-sizing:border-box;flex-wrap:wrap;justify-content:${a}}.lm-b{background:${t.b};border:${t.bW==='0'?'none':'1px solid '+t.bB};padding:10px;cursor:pointer;border-radius:8px;display:inline-flex;align-items:center;justify-content:center;transition:all .25s ease;color:${t.c};box-shadow:${c.theme==='minimal'?'none':'0 1px 2px rgba(0,0,0,0.05)'};flex:0 1 auto;min-width:40px;max-width:100%}.lm-b:hover{background-color:${t.hB};color:${t.hC};transform:translateY(-1px)}.lm-b.act{background-color:${t.aBg};border-color:${t.aB};color:${t.aC};font-weight:bold;box-shadow:${c.theme==='minimal'?'none':'0 4px 6px -1px rgba(0,0,0,0.05)'};pointer-events:none!important;cursor:default!important;transform:none!important}@media(max-width:576px){.lm-c{justify-content:center;gap:12px}.lm-b{padding:12px;flex:1;max-width:120px}}.lm-b svg{width:20px;height:20px;fill:currentColor;flex-shrink:0}.lm-wrap{position:relative;width:100%;box-sizing:border-box}.lm-nav-b{position:absolute;top:50%;z-index:999;display:none;opacity:0.6;visibility:visible;box-shadow:0 4px 10px rgba(0,0,0,0.15)!important;transition:opacity .3s ease,visibility .3s ease,transform .3s ease}.lm-nav-b:hover{background-color:${t.hB};opacity:1!important}.lm-nav-b.prev{left:-15px;transform:translateY(-50%) translateX(-3px)}.lm-nav-b.next{right:-15px;transform:translateY(-50%) translateX(3px)}.lm-wrap:hover .lm-nav-b{opacity:1;visibility:visible}.lm-wrap:hover .lm-nav-b.prev{transform:translateY(-50%) translateX(0)}.lm-wrap:hover .lm-nav-b.next{transform:translateY(-50%) translateX(0)}@media(max-width:768px){.lm-nav-b{opacity:0.9!important;visibility:visible!important;transform:translateY(-50%)!important}.lm-nav-b.prev{left:5px}.lm-nav-b.next{right:5px}.lm-nav-b{padding:8px;min-width:35px}}#${c.listaId}.is-car~.lm-nav-b{display:inline-flex!important}#${c.listaId}{box-sizing:border-box;width:100%;max-width:100%}#${c.listaId}.is-grid{display:grid!important;grid-template-columns:repeat(auto-fit,minmax(300px,1fr))!important;gap:${gGrid}!important}#${c.listaId}.is-list{display:flex!important;flex-direction:column!important;gap:${gList}!important}#${c.listaId}.is-list .${c.itens}{width:100%!important;max-width:100%!important;box-sizing:border-box!important}#${c.listaId}.is-tree{display:flex!important;flex-direction:column!important;gap:12px!important;padding-left:20px!important;border-left:2px dashed #cbd5e1!important}#${c.listaId}.is-tree .${c.itens}{width:100%!important;max-width:100%!important;box-sizing:border-box!important;position:relative!important}#${c.listaId}.is-tree .${c.itens}::before{content:''!important;position:absolute!important;left:-22px!important;top:50%!important;width:16px!important;height:2px!important;background:#cbd5e1!important}@media(max-width:576px){#${c.listaId}.is-tree{padding-left:10px!important;border-left:1px dashed #cbd5e1!important}#${c.listaId}.is-tree .${c.itens}::before{left:-11px!important;width:8px!important}}#${c.listaId}.is-car{display:flex!important;flex-direction:row!important;overflow-x:auto!important;scroll-snap-type:x mandatory!important;gap:${gGrid}!important;scroll-behavior:smooth!important;scrollbar-width:none!important;padding:10px 0!important;width:100%!important}#${c.listaId}.is-car::-webkit-scrollbar{display:none!important}#${c.listaId}.is-car .${c.itens}{flex:0 0 300px!important;scroll-snap-align:start!important;max-width:none!important;box-sizing:border-box!important}@media(max-width:480px){#${c.listaId}.is-car .${c.itens}{flex:0 0 85vw!important}}@keyframes lmFade{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}.${c.itens}.lm-anim{animation:lmFade 0.35s cubic-bezier(0.4,0,0.2,1) forwards!important}`;document.head.appendChild(s);} function fs_lismode(c){const l=document.getElementById(c.listaId);if(!l)return;const t=fs_lismode_gettheme(c.theme);fs_lismode_injectstyles(c,t);const w=document.createElement('div');w.className='lm-wrap';l.parentNode.insertBefore(w,l);w.appendChild(l);const d=document.createElement('div');d.className='lm-c';w.parentNode.insertBefore(d,w);const g=document.createElement('button');g.className='lm-b';g.innerHTML='<svg viewBox="0 0 24 24"><path d="M4 4h4v4H4V4zm6 0h4v4h-4V4zm6 0h4v4h-4V4zM4 10h4v4H4v-4zm6 0h4v4h-4v-4zm6 0h4v4h-4v-4zM4 16h4v4H4v-4zm6 0h4v4h-4v-4zm6 0h4v4h-4v-4z"/></svg>';const b=document.createElement('button');b.className='lm-b';b.innerHTML='<svg viewBox="0 0 24 24"><path d="M4 10.5c-.83 0-1.5.67-1.5 1.5s.67 1.5 1.5 1.5 1.5-.67 1.5-1.5-.67-1.5-1.5-1.5zm0-6c-.83 0-1.5.67-1.5 1.5s.67 1.5 1.5 1.5 1.5-.67 1.5-1.5-.67-1.5-1.5-1.5zm0 12c-.83 0-1.5.68-1.5 1.5s.68 1.5 1.5 1.5 1.5-.68 1.5-1.5-.67-1.5-1.5-1.5zM7 19h14v-2H7v2zm0-6h14v-2H7v2zm0-7v2h14V6H7z"/></svg>';const tr=document.createElement('button');tr.className='lm-b';tr.innerHTML='<svg viewBox="0 0 24 24"><path d="M22 11V3h-7v3H9V3H2v8h7V8h2v10h4v3h7v-8h-7v3h-2V8h2v3h7zm-15-6h3v4H7V5zm-5 0h3v4H2V5zm15 12h3v4h-3v-4z"/></svg>';const cr=document.createElement('button');cr.className='lm-b';cr.innerHTML='<svg viewBox="0 0 24 24"><path d="M7 19h10V5H7v14zm-5-2h3V7H2v10zm17 0h3V7h-3v10zM12 9l-3 3h2v3h2v-3h2l-3-3z"/></svg>';d.appendChild(g);d.appendChild(b);d.appendChild(tr);d.appendChild(cr);const pB=document.createElement('button');pB.className='lm-b lm-nav-b prev';pB.innerHTML='<svg viewBox="0 0 24 24"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg>';const nB=document.createElement('button');nB.className='lm-b lm-nav-b next';nB.innerHTML='<svg viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>';w.appendChild(pB);w.appendChild(nB);function getScrollAmount(){const firstItem=l.querySelector('.'+c.itens);return firstItem?firstItem.offsetWidth+parseInt(window.getComputedStyle(l).gap||0):330;}pB.addEventListener('click',()=>l.scrollBy({left:-getScrollAmount(),behavior:'smooth'}));nB.addEventListener('click',()=>l.scrollBy({left:getScrollAmount(),behavior:'smooth'}));function fs_lismode_toggleview(m){const items=l.querySelectorAll('.'+c.itens);items.forEach(i=>i.classList.remove('lm-anim'));l.classList.remove('is-grid','is-list','is-tree','is-car');g.classList.remove('act');b.classList.remove('act');tr.classList.remove('act');cr.classList.remove('act');if(m==='list'){l.classList.add('is-list');b.classList.add('act');}else if(m==='tree'){l.classList.add('is-tree');tr.classList.add('act');}else if(m==='carousel'){l.classList.add('is-car');cr.className='lm-b act';}else{l.classList.add('is-grid');g.classList.add('act');}localStorage.setItem('fs_layout_mode',m);if(items.length>0){void l.offsetWidth;items.forEach(i=>i.classList.add('lm-anim'));}if(typeof c.callback==='function')c.callback(m);}g.addEventListener('click',function(){fs_lismode_toggleview('grid');});b.addEventListener('click',function(){fs_lismode_toggleview('list');});tr.addEventListener('click',function(){fs_lismode_toggleview('tree');});cr.addEventListener('click',function(){fs_lismode_toggleview('carousel');});const sM=localStorage.getItem('fs_layout_mode')||'grid';fs_lismode_toggleview(sM);}
...
function share_txtD_all(fg){  
	...
	fsummary_Add({ tpp:".", seletor:"shareon_main", texto:share_btsD_all_yd, item:'<svg fill="none" width="24px" height="24px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M21.707,11.293l-8-8A.99991.99991,0,0,0,12,4V7.54492A11.01525,11.01525,0,0,0,2,18.5V20a1,1,0,0,0,1.78418.62061,11.45625,11.45625,0,0,1,7.88672-4.04932c.0498-.00635.1748-.01611.3291-.02588V20a.99991.99991,0,0,0,1.707.707l8-8A.99962.99962,0,0,0,21.707,11.293ZM14,17.58594V15.5a.99974.99974,0,0,0-1-1c-.25488,0-1.2959.04932-1.56152.085A14.00507,14.00507,0,0,0,4.05176,17.5332,9.01266,9.01266,0,0,1,13,9.5a.99974.99974,0,0,0,1-1V6.41406L19.58594,12Z"/></svg>' });
}
}
...
function gerenciarAreaFav(idioma){ if(typeof initBookmarkManager === 'function'){   var divPai=document.getElementById("areafav");if(!divPai){divPai=document.createElement("div");divPai.id="areafav";var divFilho=document.createElement("div");divFilho.id="areafavlist";var style=document.createElement("style");style.textContent=".bkm-main-wrapper, .bkm-main-wrapper div, .bkm-main-wrapper div button { pointer-events: none; display: none; }";divPai.appendChild(style);divPai.appendChild(divFilho);document.body.appendChild(divPai);  var initBookmarkManager_mo=initBookmarkManager({containerId:"areafavlist",targetMode:"modal",lang:idioma==="pt"?"pt":"en",startMaximized:true});  bookmarkmanages_pre={title:idioma==="pt"?"Gerenciador de Favoritos":"Bookmark Manager",icon:'<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>',open:initBookmarkManager_mo.abrir };  }  }  }
function gerenciarAreaFav_start(idioma){   gerenciarAreaFav(idioma);    bookmarkmanages_pre={title:idioma==="pt"?"Gerenciador de Favoritos":"Bookmark Manager",icon:'<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>',open:typeof initBookmarkManager_mo?.abrir === "function" ? initBookmarkManager_mo.abrir : null };   document.body.onload=function(){    bookmarkmanages_pre={title:idioma==="pt"?"Gerenciador de Favoritos":"Bookmark Manager",icon:'<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>',open:typeof initBookmarkManager_mo?.abrir === "function" ? initBookmarkManager_mo.abrir : null };    };   }
...
function showPageCard_info(df) { return (!df || !df.text || !df.text.trim()) ? "" : '<div class="info-alert' + (df.mode && df.mode.trim() ? ' ' + df.mode.trim() : '') + '">' + (df.svg && df.svg.trim() ? df.svg : (df.icon === true ? '<svg class="info-icon" viewBox="0 0 24 24" width="18" height="18"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z"/></svg>' : '')) + '<p class="info-text">' + (df.title && df.title.trim() ? '<strong>' + df.title + ':</strong> ' : '') + df.text + '</p></div>'; }
...
var dalickstens_fraxudr = "";
if (op.itens[is].links[i].type == "button") {
    if (op.itens[is].links[i].varName && op.itens[is].links[i].varName != "") {
        dalickstens_fraxudr = " fs_VarN='" + op.itens[is].links[i].varName + "' "; 
    }
}


 dalickstens_fra=dalickstens_fra+"<"+op.itens[is].links[i].type+dalickstens_fraxudr+hredadd_itens_ocul+" class='"+dadd_itens_fra_co["class_b"]+hredaiconns_octyoeul+"' "+hredadd_itens_fradd+">"+hredaiconns_ocul+"  "+op.itens[is].links[i].text+"  <"+"/"+op.itens[is].links[i].type+">";  
        }
        }
    }
